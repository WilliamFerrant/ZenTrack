...
function ToastItem({ toast, onClose }: ToastItemProps) {
  ...
  const getToastStyles = () => {
    const base = 'max-w-sm w-full bg-white shadow-lg rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5'

    switch (toast.type) {
      case 'success':
        return `${base} border-l-4 border-green-400`
      case 'error':
        return `${base} border-l-4 border-red-400`
      case 'warning':
        return `${base} border-l-4 border-yellow-400`
      case 'info':
        return `${base} border-l-4 border-blue-400`
      default:
        return base
    }
  }
